...
const AddAccountModal = ({ isOpen, onClose }) => {
  ...
  const handleOAuthStart = () => {
    ...
    window.open(`${API_BASE}/v1/auth/login`, '_blank', 'width=600,height=700');
    setStatus('loading');
    ...
    <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4">
      ...
      <div className="w-full max-w-lg bg-background border border-glass-border rounded-2xl shadow-2xl overflow-hidden relative z-10 animate-in fade-in zoom-in-95 duration-300">
        ...
        <div className="px-6 py-4 border-b border-glass-border flex justify-between items-center bg-foreground/[0.02]">
          ...
          <button onClick={onClose} className="text-foreground/40 hover:text-foreground transition-colors">✕</button>
        </div>
            ...
            {activeTab === 'oauth' ? (
              <div className="space-y-6">
                <div className="text-center py-6 bg-foreground/[0.02] border border-dashed border-glass-border rounded-2xl">
                  ...
                  <p className="text-foreground/40 text-xs px-10">{t('accounts.viaBrowserDesc')}</p>
                  <button 
                ...
                <div className="space-y-3">
                  <div className="flex items-center gap-2 text-xs text-foreground/30 font-medium px-1 uppercase tracking-wider">
                  ...
                  <div className="flex gap-2">
                    <div className="relative flex-1 group">
                      <Link2 className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-foreground/20 group-focus-within:text-blue-500 transition-colors" />
                      <input 
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center">
                   <span className="text-xs text-foreground/40 font-medium">{t('accounts.batchPasteTokens')}</span>
